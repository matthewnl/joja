...
int Sys_GetCPUSpeedOld()
{
	...
	int firsttry = floor(static_cast<double> (coarse + 25) / 50) * 50;
...
#define ROUND_THRESHOLD		6
	...
	do {	// This do loop runs up to 20 times or until the average of the previous 
		...
		total = ( freq + freq2 + freq3 ); // Total last three frequency calculations
	          ...
	          (tries < 20)&&
	          ((abs(static_cast<double> (3 * freq) -total) > 3*TOLERANCE )||
	           (abs(static_cast<double> (3 * freq2)-total) > 3*TOLERANCE )||
	           (abs(static_cast<double> (3 * freq3)-total) > 3*TOLERANCE )));	
