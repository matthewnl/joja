...
int PM_AnimLength( int index, animNumber_t anim )
{
	...
	return level.knownAnimFileSets[index].animations[anim].numFrames * fabs(static_cast<double> (level.knownAnimFileSets[index].animations[anim].frameLerp));
}
...
*/
		...
		{//FIXME: allow to set a specific time?
			if ( timeScaleMod != 1.0 )
			{
				PM_SetTorsoAnimTimer( gent, torsoAnimTimer, (animations[anim].numFrames - 1) * fabs(static_cast<double> (animations[anim].frameLerp)) / timeScaleMod );
			}
			else if ( setAnimFlags & SETANIM_FLAG_HOLDLESS )
			{	// Make sure to only wait in full 1/20 sec server frame intervals.
				int dur;
				
				dur = (animations[anim].numFrames -1) * fabs(static_cast<double> (animations[anim].frameLerp));
				//dur = ((int)(dur/50.0)) * 50;
				//dur -= blendTime;
				if (dur > 1)
				{
				...
				{
					PM_SetTorsoAnimTimer( gent, torsoAnimTimer, fabs(static_cast<double> (animations[anim].frameLerp)) );
				}
			}
			else
			{
				PM_SetTorsoAnimTimer( gent, torsoAnimTimer, (animations[anim].numFrames ) * fabs(static_cast<double> (animations[anim].frameLerp)) );
			}
		}
	}
...
*/
		...
		{//FIXME: allow to set a specific time?
			if ( timeScaleMod != 1.0 )
			{
				PM_SetLegsAnimTimer( gent, legsAnimTimer, (animations[anim].numFrames - 1) * fabs(static_cast<double> (animations[anim].frameLerp)) / timeScaleMod );
			}
			else if ( setAnimFlags & SETANIM_FLAG_HOLDLESS )
			{	// Make sure to only wait in full 1/20 sec server frame intervals.
				int dur;
				...
				{
					PM_SetLegsAnimTimer( gent, legsAnimTimer, fabs(static_cast<double> (animations[anim].frameLerp)) );
				}
			}
			else
			{
				PM_SetLegsAnimTimer( gent, legsAnimTimer, (animations[anim].numFrames ) * fabs(static_cast<double> (animations[anim].frameLerp)) );
			}
		}
	}
