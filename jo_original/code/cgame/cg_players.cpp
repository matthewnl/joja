...
qboolean CG_RunLerpFrame( clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float fpsMod, int entNum ) {
	...
	if ( cg.time >= lf->frameTime ) 
	{
		...
		{
			animFrameTime = fabs(static_cast<double> (anim->frameLerp));
...
void CG_PlayerAnimSounds( int animFileIndex, qboolean torso, int oldFrame, int frame, int entNum )
{
	...
	if ( fabs(static_cast<double> (oldFrame-frame)) > 1 && cg_reliableAnimSounds.integer )
	...
	for (i=0;i<MAX_ANIM_SOUNDS;++i)
	{
		...
		else if ( fabs(static_cast<double> (oldFrame-frame)) > 1 && cg_reliableAnimSounds.integer )
		{//given a range, see if keyFrame falls in that range
			if ( inSameAnim )
			{//if changed anims altogether, sorry, the sound is lost
				if ( fabs(static_cast<double> (oldFrame-animSounds[i].keyFrame)) <= 3
					 || fabs(static_cast<double> (frame-animSounds[i].keyFrame)) <= 3 )
